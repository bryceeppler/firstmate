...
[`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns new-spawn authorization, runtime auto-detection, explicit selection precedence, and opt-out notices.
...
t3code reads the T3 server's own session status, and both its native `busy` and `idle` are accepted ahead of harness records because no shell sits in front of the agent.
...
For capable Herdr and T3 sessions, the same watcher replaces its terminal sleep with a bounded native event wait.
The [Herdr](herdr-backend.md#push-events-and-polling-fallback) and [T3](t3code-backend.md#push-events-and-polling-fallback) guides own their mechanisms and capability gates, while [runtime backend verification](verification/runtime-backends.md) owns active evidence.
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified process classifiers and T3 has a native session classifier for secondmate recovery, Zellij remains unverified, and Orca and cmux do not support secondmate spawns.
...
The experimental T3 backend owns the agent session over HTTP and WebSocket while Treehouse owns the task worktree; [`t3code-backend.md`](t3code-backend.md) owns current setup and limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#t3-code) owns the live probe evidence.
Codex App support is recorded in `docs/codex-app-backend.md`; it is not selectable as a runtime backend.

## Worktrees, not branches in your checkout

Crewmates never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, cmux, and t3code tasks, while Orca creates its own worktrees for `backend=orca`.
...
Fleet state lives in each task's recorded runtime backend, no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
